...
import { deleteAdminBookmark, fetchAdminBookmarks } from "@/lib/api";
...
export function BookmarksPage() {
  ...
  const [rows, setRows] = useState<
    { userId: string; postId: string; createdAt: string }[] | null
  >(null);
  ...
  async function remove(userId: string, postId: string) {
    ...
    try {
      ...
      toast.success("Đã gỡ khỏi thư viện");
      await load();
    } catch (e: unknown) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-2xl font-semibold">Đã lưu</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Bài đã lưu vào thư viện (nút «Lưu» trên blog). Tổng: {total}.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Tuỳ chọn userId (người lưu) và/hoặc postId (bài được lưu).
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-wrap gap-4">
          <div className="space-y-2">
            <Label htmlFor="buid">Người lưu (userId)</Label>
            <Input
              id="buid"
              value={userIn}
          ...
          <div className="space-y-2">
            <Label htmlFor="bpid">Bài viết (postId)</Label>
            <Input
              id="bpid"
              value={postIn}
        ...
        <CardContent>
          ...
          {rows && rows.length > 0 && (
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>Người lưu</TableHead>
                  <TableHead>Bài viết</TableHead>
                  <TableHead>Lưu lúc</TableHead>
